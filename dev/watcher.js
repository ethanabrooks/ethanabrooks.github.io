...
// system to listen for build events. See package.json's `start` script and
// `./node_modules/.bin/bsb --help`

// Btw, if you change this file and reload the page, your browser cache _might_
// not pick up the new version. If you're in Chrome, do Force Reload.

let websocketReloader;
...
function setUpWebSocket() {
  if (websocketReloader != null && websocketReloader.readyState === 1) return;

  try {
    websocketReloader = new WebSocket(`ws://localhost:${webSocketPort}`);
    websocketReloader.onmessage = (message) => {
      let newData = JSON.parse(message.data).LAST_SUCCESS_BUILD_STAMP;
      if (newData <= LAST_SUCCESS_BUILD_STAMP) return;
      LAST_SUCCESS_BUILD_STAMP = newData;
      localStorage.setItem(
        "LAST_SUCCESS_BUILD_STAMP",
        LAST_SUCCESS_BUILD_STAMP
      );
      // Refresh the page! This will naturally re-run everything, including
      // our moduleserve which will re-resolve all the modules. No stable
      // build!
